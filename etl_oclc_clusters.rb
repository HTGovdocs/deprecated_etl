...
def build_record ids, o_str_id, e_str_id
  ...
  if e_str_id 
    rec['enumchron_display'] = get_enumchron(e_str_id)
  else
  ...
  ids.each do | id | 
    ...
    if src_file =~ /zeph/ and !rec.has_key? 'text'
      ...
      rec.merge!(@@indexer.map_record(base_marc))
    end
  end
...
header = cluster_report.readline
cluster_report.each_with_index do | line, line_num |
  ...
  solr_index build_record(ids, o_str_id, e_str_id) 
